...
// Pending = every open form awaiting confirmation, ANY date (no quarter window).
// Here: ESF 6/25 Pending Payroll 800 + PSF 7/15 Pending Office Manager 600, which sits BEYOND the
// quarter and would have been missed under the old (today, qEnd] window. Cancelled rows are already
// stripped by unifyStarts, so they never reach this filter.
assert.strictEqual(r.scorecard.pendingStarts.count, 2, "pending = open forms with spread, any date incl. beyond qtr");
assert.strictEqual(r.scorecard.pendingStarts.avgSpread, 700, "pending avg = (800 ESF + 600 PSF) / 2");
...
// Dump-in stays quarter-scoped, so it no longer equals netNew + pending now that pending spans all
// dates: the PSF 7/15 pending row is outside the quarter and counts in pending but not dump-in.
assert.strictEqual(r.scorecard.dumpIn.count, 4, "dump-in = non-cancelled starts dated in the quarter");
// --- $1,250 Raffle ---
...
// Both Pending-status rows now count regardless of date: the 5/01 one that started and was never
// confirmed (exactly the backlog the tile should surface) and the future 6/26 one. The "Started"
// row is confirmed, so it stays out.
assert.strictEqual(rs.scorecard.pendingStarts.count, 2, "both unconfirmed rows count, past date included");
assert.strictEqual(rs.scorecard.pendingStarts.avgSpread, 1650, "pending avg = (2000 + 1300) / 2");
